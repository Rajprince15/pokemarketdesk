      ...
      <main className="container py-8">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground">AI-powered buy/sell signals based on market trends</p>
          </div>
        </div>
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 mb-8 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium mb-1">Algorithmic Predictions</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-success/10 border border-success/20 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-semibold text-success">Buy Signals</h3>
            </div>
            <p className="text-3xl font-bold text-success">{buySignals.length}</p>
            <p className="text-xs text-muted-foreground mt-1">Cards showing upward momentum</p>
          </div>

          <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-2">
              <TrendingDown className="h-5 w-5 text-destructive" />
              <h3 className="font-semibold text-destructive">Sell Signals</h3>
            </div>
            <p className="text-3xl font-bold text-destructive">{sellSignals.length}</p>
            <p className="text-xs text-muted-foreground mt-1">Cards showing downward trends</p>
          </div>

          <div className="bg-secondary/50 border border-border/50 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-2">
              <Minus className="h-5 w-5 text-muted-foreground" />
              <h3 className="font-semibold">Hold Signals</h3>
            </div>
            <p className="text-3xl font-bold">{holdSignals.length}</p>
            <p className="text-xs text-muted-foreground mt-1">Cards with stable conditions</p>
          </div>
        </div>
            ...
            >
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-shrink-0">
                  <div className={`w-32 h-40 rounded-lg ${typeColors[prediction.card.type]} p-2 flex items-center justify-center`}>
                    <img
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-bold text-lg">{prediction.card.name}</h3>
                        <Badge variant="outline" className={`text-xs ${typeColors[prediction.card.type]}`}>
                          {prediction.card.type}
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground">{prediction.card.set}</p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-2xl font-bold">${prediction.card.price.toLocaleString()}</p>
                  ...
                  <div className="bg-secondary/30 rounded-lg p-4">
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-sm mb-2">{prediction.recommendation}</p>
